...
declare global {
  // `var` (not let/const) is required for ambient global augmentation so the
  // symbol is reachable as a bare identifier after esbuild's define replacement.
...
if (typeof (window as unknown as Record<string, unknown>)['OBSYNC_DEV_BUILD'] === 'undefined') {
  // Coerce via `String() === "true"` so the post-bundle replacement
  // ("true" or "false") still yields a real boolean.
  try {
    (window as unknown as Record<string, unknown>)['OBSYNC_DEV_BUILD'] =
      String(OBSYNC_DEV_BUILD) === 'true';
  } catch {
    (window as unknown as Record<string, unknown>)['OBSYNC_DEV_BUILD'] = false;
  }
}

/** Reads the current flag value (host-controlled in tests). */
export function isDevBuild(): boolean {
  return Boolean((window as unknown as Record<string, unknown>)['OBSYNC_DEV_BUILD']);
}

/** Test-only setter — production code must never call this. */
export function __setDevBuildForTests(flag: boolean): void {
  (window as unknown as Record<string, unknown>)['OBSYNC_DEV_BUILD'] = flag;
}
